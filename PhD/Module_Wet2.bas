...
Public Function CalculateWetSeason(ByVal I As Double, ByVal S As Double, ByVal R As Double, ByVal temp As Double, ByVal t As Double, ByVal A As Double, ByVal W As Double, ByVal d As Double, ByVal h As Double) As Double
    ' Wet Season Formula: V = 15.4 + 0.0003I + 5.24S + 0.108R + 4.43T - 2.03t + 0.0003A + 0.0495W + 0.0012d - 0.007h
    
 Dim V As Double
    V = 15.4 + (0.0003 * I) + (5.24 * S) + (0.108 * R) + (4.43 * temp) - (2.03 * t) + (0.0003 * A) + (0.0495 * W) + (0.0012 * d) - (0.007 * h)
    
    CalculateWetSeason = V
End Function

' --- Main Batch Processing Subroutine ---
Public Sub ProcessBatchWet(ByVal filePath As String)
    ...
    ThisWorkbook.Sheets("Results_Wet_Season").Delete
    On Error GoTo 0
    Application.DisplayAlerts = True
    
    Set wsResult = ThisWorkbook.Sheets.Add(After:=ThisWorkbook.Sheets(ThisWorkbook.Sheets.Count))
    wsResult.Name = "Results_Wet_Season"
    ...
    wsResult.Cells(1, 10).Value = "Lnad Surface Temperature (°C)"
    ...
    Do While Not EOF(iFile)
        ...
        WetVolume = CalculateWetSeason(I, S, R, temp, t, A, W, d, h)
        ...
        wsResult.Cells(lRow, 16).Value = WetVolume
